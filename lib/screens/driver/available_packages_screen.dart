  ...
  Future<void> _acceptPackage(Package package) async {
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Colis accepté ! Retrouvez-le dans "Mes Livraisons".')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Erreur lors de l\'acceptation du colis.')),
      );
    }
  }
          ...
          actionButton = ElevatedButton.icon(
           ...
           actionButton = Container(
             ...
             child: const Row(
               ...
               children: [Icon(Icons.done_all, color: Colors.green), SizedBox(width: 8), Text('Livré')],
             ),
           );
        }
             ...
             Row(
              children: [
                ...
                Container(
                  ...
                  child: Text(package.weight, style: TextStyle(color: Colors.blue.shade800, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 12),
            _buildAddressRow(Icons.location_on, Colors.green, package.pickupAddress.label),
            const SizedBox(height: 8),
            _buildAddressRow(Icons.flag, Colors.red, package.deliveryAddress.label),
